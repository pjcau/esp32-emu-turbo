...
def test_T6_power_chain(net_pads, ref_pads):
    ...
    # BAT+ must reach: Q1 (P-MOSFET source), L1 (inductor), SW16
    # J3.1 is on BAT_IN (pre-RPP); BAT+ starts at Q1's SOURCE, because the
    # cell sits on the drain — R31-HIGH-1. Which pad is which is enforced by
    # verify_polarity's strict Q1 entry, not here.
    ...
    check("T6", "BAT+ reaches inductor (L1)", "L1" in batp_comps,
          f"BAT+ components: {sorted(batp_comps)}")
    # BAT_IN must reach: J3 (battery), Q1 (P-MOSFET drain)
