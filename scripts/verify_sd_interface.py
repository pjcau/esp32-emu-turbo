...
def analyze_sd_interface(cache):
    ...
    if cd_name in CD_FEATURE_NETS:
        ...
        results.append(("INFO",
                        ...
                        f"-- not a card-detect feature: no firmware reads it, and the "
                        f"net is there because that track crosses the pad "
                        f"(see CLAIM-006). Firmware must poll card presence"))
    else:
        results.append(("INFO",
