...
def _silkscreen_labels():
    ...
    parts.append(P.gr_text("MENU", px, py - 5, "F.Fab", 1.0))

    # LED labels (front side, bottom-left)
    px, py = enc_to_pcb(*LED_CHARGE_ENC)
    parts.append(P.gr_text("CHG", px, py + 3, "F.Fab", 1.0))
    px, py = enc_to_pcb(*LED_FULL_ENC)
    parts.append(P.gr_text("FULL", px, py + 3, "F.Fab", 1.0))
    ...
    parts.append(P.gr_text("USB-C", px, py - 6, "B.Fab", 1.0))
    px, py = enc_to_pcb(*SD_ENC)
    parts.append(P.gr_text("SD", px, py - 8, "B.Fab", 1.0))

    # Battery connector label (back side)
    px, py = enc_to_pcb(*JST_BAT_ENC)
    parts.append(P.gr_text("BATT", px, py - 5, "B.Fab", 1.0))

    # FPC display connector label (back side)
    px, py = enc_to_pcb(*FPC_ENC)
    parts.append(P.gr_text("LCD", px, py - 14, "B.Fab", 1.0))

    # Power switch label (back side)
    px, py = enc_to_pcb(*PWR_SWITCH_ENC)
    parts.append(P.gr_text("PWR", px, py - 5, "B.Fab", 1.0))

    # Speaker label (back side)
    px, py = enc_to_pcb(*SPEAKER_ENC)
    parts.append(P.gr_text("SPEAKER", px, py, "B.Fab", 1.0))

    # Shoulder button labels (back side)
    px, py = enc_to_pcb(*SHOULDER_L_ENC)
    parts.append(P.gr_text("L", px, py + 5, "B.Fab", 1.0))
    px, py = enc_to_pcb(*SHOULDER_R_ENC)
    parts.append(P.gr_text("R", px, py + 5, "B.Fab", 1.0))
    ...
    px, py = enc_to_pcb(*SHOULDER_L_ENC)
    parts.append(P.gr_text("L", px, py + 5, "B.SilkS", 1.0))
    px, py = enc_to_pcb(*SHOULDER_R_ENC)
