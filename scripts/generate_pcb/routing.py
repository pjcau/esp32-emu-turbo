...
def _mh_detour_h(x1, y, x2, layer, width, net):
    ...
    for cx, cy, cr in _KEEPOUT_CIRCLES:
        ...
        # JLCDFM FIX: AR=0.20mm with VIA_STD (0.60/0.20).
        via_sz, via_dr = VIA_STD, VIA_STD_DRILL
...
def _power_traces():
    ...
    if sd_vss:
        ...
        parts.append(_via_net(sd_vss[0], sd_vss[1], n_gnd, size=VIA_STD, drill=VIA_STD_DRILL))
    ...
    if r16_p1:
        ...
        # DFM FIX (KiBot external): via at (115.95, 54.5) gap=0.15mm to C18 pad 2
        # at (117.05, 55) size 1.0x1.3mm. Shift via LEFT by 1mm to x=114.95.
...
def _display_traces():
    ...
    for idx, gpio, fpc_pin, net_name in all_lcd:
        ...
        # DFM FIX: idx=0 via at (140.8,5.0) had 0.0mm gap to SD_MOSI vert at x=141.2.
        # Shift to 140.7: gap = |141.2-140.7|-0.3-0.1 = 0.1mm ✓
        if idx < 10:
            apx = round(140.7 - idx * 0.70, 4)   # RIGHT: 140.7, 140.0, ..., 134.4
        else:
        ...
        if is_bottom:
            ...
            if via_inside_gnd:
                ...
                parts.append(_via_net(epx, bridge_entry_y, net, size=VIA_STD, drill=VIA_STD_DRILL))
                ...
                parts.append(_seg(epx, escape_y, col_x, escape_y,
                                  "F.Cu", W_DATA, net))
                parts.append(_via_net(col_x, escape_y, net, size=VIA_STD, drill=VIA_STD_DRILL))
            else:
                # 1. B.Cu vertical from pad up to stagger level
                parts.append(_seg(epx, epy, epx, stagger_y,
                                  "B.Cu", W_DATA, net))
                parts.append(_via_net(epx, stagger_y, net, size=VIA_STD, drill=VIA_STD_DRILL))

                # 2. F.Cu horizontal to col_x (detour around mounting holes)
                parts.extend(_mh_detour_h(epx, stagger_y, col_x,
                                          "F.Cu", W_DATA, net))
                parts.append(_via_net(col_x, stagger_y, net, size=VIA_STD, drill=VIA_STD_DRILL))
        else:
            ...
            parts.append(_seg(epx, epy, via1_x, epy,
                              "B.Cu", W_DATA, net))
            parts.append(_via_net(via1_x, epy, net, size=VIA_STD, drill=VIA_STD_DRILL))

            # F.Cu horizontal to col_x (detour around mounting holes)
            parts.extend(_mh_detour_h(via1_x, epy, col_x,
                                      "F.Cu", W_DATA, net))
            parts.append(_via_net(col_x, epy, net, size=VIA_STD, drill=VIA_STD_DRILL))
        ...
        parts.append(_seg(col_x, from_y, col_x, bypass_y,
                          "B.Cu", W_DATA, net))
        parts.append(_via_net(col_x, bypass_y, net, size=VIA_STD, drill=VIA_STD_DRILL))
    ...
    if pos5:
        ...
        parts.append(_seg(px, py, stub_x, py, "B.Cu", 0.4, n_gnd))
...
def _usb_traces():
    ...
    parts.append(_seg(dm_col_x, _dm_fcu_bridge_y, dm_col_x, dm_y,
                       "B.Cu", W_DATA, n_dm))
...
def _button_traces():
    ...
    for i, b in enumerate(btn_data):
        if i <= 5:
            ...
            # DFM FIX: moved from 74.52 to 74.46 for via-edge and via-segment clearance.
            # ch9 at 74.46: gap to ch8 F.Cu(73.955) = 74.46-73.955-0.23-0.125=0.15mm ✓
            # edge gap = 75.0-74.46-0.23=0.31mm > 0.30mm ✓
            b["chan_y"] = 74.46
    ...
    _lcd_approach_xs = [round(140.7 - k * 0.70, 4) for k in range(10)] + \
    ...
    for b in btn_data:
        ...
        # DFM FIX (KiBot external): ch8/ch9 near board edge, use VIA_MIN (0.46mm)
        # for tighter packing: AR=(0.46-0.20)/2=0.13mm JLCPCB min ✓
        _btn_via_sz = VIA_MIN if i >= 8 else VIA_TIGHT
        _btn_via_drill = VIA_MIN_DRILL if i >= 8 else VIA_TIGHT_DRILL
    ...
    # DFM FIX: moved from 73.53 to 73.43 to clear BTN_START F.Cu at y=73.955.
    # Using VIA_TIGHT (0.50mm, r=0.25) for BTN_L vias (was VIA_STD 0.60mm).
    # Gap to BTN_START F.Cu: |73.955-73.43|-0.25-0.125=0.525-0.375=0.15mm ✓
    # Gap to NPTH(38.5,72.55): trace bottom=73.305, NPTH top=73.00 → 0.305mm ✓
    # Board gap: 75.0-73.43-0.25=1.32mm ✓
    chan_y_l = 73.43
    ...
    parts.append(_via_net(sx_l, chan_y_l, net_l, size=VIA_MIN, drill=VIA_MIN_DRILL))
    ...
    parts.append(_seg(sx_l, chan_y_l, approach_l, chan_y_l,
                       "F.Cu", W_DATA, net_l))
    parts.append(_via_net(approach_l, chan_y_l, net_l, size=VIA_MIN, drill=VIA_MIN_DRILL))
...
def _passive_traces():
    ...
    for i, ref in enumerate(PULL_UP_REFS):
        ...
        parts.append(_seg(via_x, ry, via_x, via_y_pu,
                          "B.Cu", W_SIG, n_3v3))
        parts.append(_via_net(via_x, via_y_pu, n_3v3, size=VIA_STD, drill=VIA_STD_DRILL))
    ...
    for i, ref in enumerate(DEBOUNCE_REFS):
        ...
        parts.append(_seg(via_x_db, cy, via_x_db, cy + 2,
                          "B.Cu", W_SIG, n_gnd))
        parts.append(_via_net(via_x_db, cy + 2, n_gnd, size=VIA_STD, drill=VIA_STD_DRILL))
    ...
    if c17_p1:
        ...
        # DFM: LCD_D4 B.Cu vert at x=111.50 (w=0.20). Gap at W_PWR_HIGH(0.76)=0.07mm < 0.15mm.
        # Use 0.50mm: gap = 0.55-0.25-0.10 = 0.20mm ✓
