...
def _bom_refs() -> Set[str]:
    """Designators in the generated JLCPCB BOM (placed or not)."""
    import csv as _csv
    path = REPO / "hardware" / "kicad" / "jlcpcb" / "bom.csv"
    refs: Set[str] = set()
    if not path.exists():
        return refs
...
def check_ref_coverage(
    ...
    # The CPL is not the whole BOM. A part marked DO NOT PLACE is in the
    # BOM (so it can be sourced and so the land is identifiable) and
    # deliberately absent from the CPL (so JLCPCB never populates it), and
    # it is still a real symbol on a real sheet — SW17, the manual KEY
    # wake button, is the first of these. Reading the CPL alone reported
    # it as a schematic ref with no BOM entry, which is the opposite of
    # true. Both files are read now; the CPL keeps deciding what is
    # PLACED, which is a different question and is checked in
    # verify_bom_cpl_pcb.
    bom_refs = _bom_refs()
    known_refs = cpl_refs | bom_refs
    sch_refs = set(sch.keys())

    # Dynamic-ref families generated by loops in board.py / jlcpcb_export.py
    # (SW1-SW13, SW15, SW14, R4-R15 pull-ups, C5-C16 debounce caps,
