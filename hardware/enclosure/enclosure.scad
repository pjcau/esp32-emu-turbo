...
dpad_x = -55;           // X offset from center
dpad_y = 5;             // Y offset from center
dpad_arm_len = 12;
dpad_arm_w = 5;

// === ABXY parameters (right side, diamond layout) ===
abxy_x = 55;            // X offset from center
abxy_y = 5;             // Y offset from center
abxy_spacing = 13;
abxy_diam = 8;

// === Start/Select (center bottom) ===
ss_y = -28;             // Below display
ss_spacing = 22;        // Distance between Start and Select
ss_w = 10;              // Pill width
ss_h = 4;               // Pill height

// === Shoulder buttons (L/R on top edge) ===
shoulder_inset_x = 55;  // Distance from center
shoulder_w = 22;
shoulder_h = 8;
...
module top_shell() {
    ...
    difference() {
        ...
        // ABXY button cutouts (diamond layout)
        translate([abxy_x, abxy_y, 0])
        abxy_diamond(abxy_spacing, abxy_diam, top_d + 1);

        // Start button cutout
        translate([-ss_spacing/2, ss_y, 0])
        pill_cutout(ss_w, ss_h, top_d + 1);

        // Select button cutout
        translate([ss_spacing/2, ss_y, 0])
        pill_cutout(ss_w, ss_h, top_d + 1);

        // L shoulder button cutout (top edge)
        translate([-shoulder_inset_x, body_h/2 - shoulder_h/2 - 1, 0])
        shoulder_button_cutout(shoulder_w, shoulder_h, top_d + 1);

        // R shoulder button cutout (top edge)
        translate([shoulder_inset_x, body_h/2 - shoulder_h/2 - 1, 0])
        shoulder_button_cutout(shoulder_w, shoulder_h, top_d + 1);
    }
...
module exploded_view() {
    ...
    // Top shell raised above
    translate([0, 0, body_d + explode_gap])
    mirror([0, 0, 1])
    top_shell();

    // Simulated display floating in between
