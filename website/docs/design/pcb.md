...
- **SW16** — power slide switch, left edge
- **SPK1** — solder pads for the 28 mm speaker (22 mm pad footprint), left area
...
**32 line items, 85 placements.** The authoritative table is generated from
...
| Q1 / D1 / F1 | RPP MOSFET / menu-combo Schottky / VBUS PTC | C10487 / C37704 / C960026 | 1 each |
| LED1, LED2 | Red LED 0805 (**both** are plain +3V3 power indicators) | C84256, C19171391 | 2 |
| SW1–SW16   | 15× SMT tact switch + 1× slide switch | C318884, C431540 | 16 |

Two references sit on the board but are **not** assembled: `R14` (DNP by design —
BTN_L must have no external pull-up) and `C28` (unused placeholder). `SPK1` is a
pad pair for the hand-soldered speaker, and `FID1`–`FID3` are fiducials. That is
85 placed + 6 non-placed = **91 references on the board**.
...
2. **BOM.csv** — `release_jlcpcb/bom.csv` (32 part-number groups, 85 placements)
3. **CPL.csv** — `release_jlcpcb/cpl.csv` (85 component placements)
...
The +5V island on In2.Cu covers the IP5306/SY8089/L1 area. Priority 1 ensures +5V wins in the overlap region. Components outside this area connect to +3V3 via the base zone.
...
3. Order 5× PCBs with SMT assembly (85 placements)
