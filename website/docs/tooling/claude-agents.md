...
graph TB
    ...
    subgraph PCB_VER["Verification 10"]
        s_ver["/verify"]
        s_dfm["/dfm-test"]
        s_drc["/drc-native"]
        s_drcaud["/drc-audit"]
...
- **Skill-based dispatch**: 35 skills map to specific workflows, reducing prompt engineering overhead
...
| **Verification (10)** | `/verify`, `/dfm-test`, `/drc-native`, `/pcb-optimize`, `/pcb-review`, `/datasheet-verify`, `/design-intent`, `/pad-analysis`, `/jlcpcb-alignment`, `/jlcpcb-validate` | 115 DFM + 9 DFA + 26 JLCPCB tests, DRC checks, layout scoring |
