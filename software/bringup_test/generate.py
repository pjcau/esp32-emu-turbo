#!/usr/bin/env python3
"""
ESP32 Emu Turbo — bring-up test firmware generator.

Emits software/bringup_test/main/bringup_test.c from the two files that are
allowed to declare hardware facts:

  * software/main/board_config.h   — every GPIO number and bus constant
  * software/sdkconfig.defaults    — the module variant (N16R8 -> flash/PSRAM)

Nothing here may invent a pin. Every number in the generated C file is either
read from those two files or derived from a rule stated next to the code that
derives it (the ESP32-S3 strapping-pin set, the N<flash>R<psram> module naming).

Usage:
    python3 software/bringup_test/generate.py            # write the C file
    python3 software/bringup_test/generate.py --check     # fail if stale
"""
...
PROJECT_ROOT = SOFTWARE.parent

BOARD_CONFIG = SOFTWARE / "main" / "board_config.h"
SDKCONFIG_DEFAULTS = SOFTWARE / "sdkconfig.defaults"
OUTPUT = HERE / "main" / "bringup_test.c"
...
LCD_CTRL_ORDER = ["LCD_CS", "LCD_RST", "LCD_DC", "LCD_WR"]
SD_ORDER = ["SD_MOSI", "SD_MISO", "SD_CLK", "SD_CS"]

# ESP32-S3 strapping pins and what each one latches at reset.
# Source: ESP32-S3 datasheet section 2.4 "Strapping Pins". This is a property
# of the silicon, not of this board, so it is a constant here; which net lands
# on each of them is read from board_config.h below.
S3_STRAPPING_PINS = {
    0:  "boot mode (LOW at reset = serial download)",
    3:  "JTAG source select",
    45: "VDD_SPI voltage (LOW at reset = 3.3 V, required by Octal PSRAM)",
    46: "ROM message print enable",
}

# The net that drives the diagnostic heartbeat LED (LED6 + R31 in the
# diagnostic-LED design). Absent from board_config.h on revisions that predate
# it — the firmware then reports led.heartbeat as a SKIP rather than pretending
# to drive a pin that carries something else.
HB_LED_NET = "LED_HB"

# Blink-code contract. docs/diagnostic-leds-roadmap.md, "Design (locked)":
#
...
#
# This list is the only place that mapping exists: the C table, the report
# lines and the documentation comment are all rendered from it. A check is
# attributed to a subsystem by the prefix of its id, so every current and
# future check in a group inherits its code without a second table to update.
#
# Code 1 is deliberately unused: one blink followed by a pause is hard to tell
# apart from the 1 Hz "alive" pattern, which is the one reading that must never
# be ambiguous.
BLINK_CODES = [
    (2, "SD",      ["sd."]),
    (3, "display", ["lcd."]),
...
def gpio_of(defines: dict[str, str], key: str) -> int | None:
    m = re.match(r"GPIO_NUM_(\d+)$", defines.get(key, ""))
    return int(m.group(1)) if m else None


def nets_without_external_pullup(text: str) -> set[str]:
    """Read the 'has NO external pull-up' notes out of board_config.h.

    The RC release-time check needs to know which button nets are expected to
    charge through a 10k/100nF network and which have nothing on them at all.
    That fact is stated in prose in board_config.h; parsing it here means the
    firmware follows the header instead of carrying a second opinion.
    """
    return set(re.findall(r"(\w+)\s*\(GPIO\d+\)\s*has NO external pull-up", text))


def parse_module_variant(text: str) -> tuple[str, int, int]:
    """Derive flash and PSRAM size from the module suffix (N16R8).

    Espressif's module naming is N<flash MiB>R<PSRAM MiB>; the variant string
    is declared in sdkconfig.defaults. No default: if the suffix is not there,
    the expected sizes are unknown and guessing them would make the check a
    tautology.
    """
    m = re.search(r"\b(ESP32-S3\s+)?N(\d+)R(\d+)\b", text)
    if not m:
        raise GenError(
            f"no N<flash>R<psram> module variant found in {SDKCONFIG_DEFAULTS}; "
            "flash/PSRAM size expectations cannot be derived"
        )
    return f"N{m.group(2)}R{m.group(3)}", int(m.group(2)), int(m.group(3))


# ── generated C tables ──────────────────────────────────────────────────

def c_tables(defines: dict[str, str], header_text: str) -> str:
    no_pullup = nets_without_external_pullup(header_text)
    variant, flash_mb, psram_mb = parse_module_variant(SDKCONFIG_DEFAULTS.read_text())

    buttons = []
    for name in BUTTON_ORDER:
        num = gpio_of(defines, name)
        if num is None:
            raise GenError(f"{name} missing or not a GPIO_NUM_x in board_config.h")
        buttons.append((name, name[4:], num, name not in no_pullup))

    lcd_data = []
    for i in range(8):
        key = f"LCD_D{i}"
        num = gpio_of(defines, key)
        if num is None:
            raise GenError(f"{key} missing from board_config.h")
        lcd_data.append((key, num))

    lcd_ctrl = []
    for key in LCD_CTRL_ORDER:
        num = gpio_of(defines, key)
        if num is None:
            raise GenError(f"{key} missing from board_config.h")
        lcd_ctrl.append((key, key[4:], num))
    ...
    for key in SD_ORDER:
        ...
        if num is None:
            raise GenError(f"{key} missing from board_config.h")
        sd_pins.append((key, key[3:], num))

    hb_gpio = gpio_of(defines, HB_LED_NET)

    i2s_dout = gpio_of(defines, "I2S_DOUT")
    usb_dp = gpio_of(defines, "USB_DP")
    usb_dn = gpio_of(defines, "USB_DN")
    if None in (i2s_dout, usb_dp, usb_dn):
        raise GenError("I2S_DOUT / USB_DP / USB_DN missing from board_config.h")

    # Every GPIO the header assigns, for the double-assignment check.
    pinmap = sorted(
        ((k, v) for k in defines if (v := gpio_of(defines, k)) is not None),
        key=lambda kv: (kv[1], kv[0]),
    )

    # Which board nets land on a strapping pin. Derived, not declared: the
    # strapping set is silicon, the net names come from board_config.h.
    gpio_to_nets: dict[int, list[str]] = {}
    for net, num in pinmap:
        gpio_to_nets.setdefault(num, []).append(net)
    straps = [
        (num, "+".join(gpio_to_nets.get(num, ["<unassigned>"])), role)
        for num, role in sorted(S3_STRAPPING_PINS.items())
    ]

    L: list[str] = []
    w = L.append

    w("/* ── Generated tables (from board_config.h) ─────────────────── */")
    w("")
    w("typedef struct {")
    w("    const char  *name;      /* short label, e.g. \"UP\"           */")
    w("    gpio_num_t   pin;")
    w("    int          num;       /* GPIO number, for the report line   */")
    w("    bool         ext_pullup;/* PCB has the 10k pull-up + 100nF    */")
    w("} btn_t;")
    w("")
    w("static const btn_t BUTTONS[] = {")
    for name, label, num, has_pu in buttons:
        note = "" if has_pu else "  /* board_config.h: NO external pull-up */"
        w(f'    {{ "{label}", {name}, {num}, {"true" if has_pu else "false"} }},{note}')
    w("};")
    w(f"#define N_BUTTONS {len(buttons)}")
    w("")
    w("static const gpio_num_t LCD_DATA[] = { "
      + ", ".join(k for k, _ in lcd_data) + " };")
    w("static const int LCD_DATA_NUM[] = { "
      + ", ".join(str(n) for _, n in lcd_data) + " };")
    w(f"#define N_LCD_DATA {len(lcd_data)}")
    w("")
    w("static const gpio_num_t LCD_CTRL[] = { "
      + ", ".join(k for k, _, _ in lcd_ctrl) + " };")
    w('static const char *LCD_CTRL_NAME[] = { '
      + ", ".join(f'"{lbl}"' for _, lbl, _ in lcd_ctrl) + " };")
    w("static const int LCD_CTRL_NUM[] = { "
      + ", ".join(str(n) for _, _, n in lcd_ctrl) + " };")
    w(f"#define N_LCD_CTRL {len(lcd_ctrl)}")
    w("")
    w("static const gpio_num_t SD_LINES[] = { "
      + ", ".join(k for k, _, _ in sd_pins) + " };")
    w('static const char *SD_LINE_NAME[] = { '
      + ", ".join(f'"{lbl}"' for _, lbl, _ in sd_pins) + " };")
    w("static const int SD_LINE_NUM[] = { "
      + ", ".join(str(n) for _, _, n in sd_pins) + " };")
    w(f"#define N_SD_LINES {len(sd_pins)}")
    w("")
    w("typedef struct { const char *net; int gpio; } pinmap_t;")
    w("static const pinmap_t PINMAP[] = {")
    for net, num in pinmap:
        w(f'    {{ "{net}", {num} }},')
    w("};")
    w(f"#define N_PINMAP {len(pinmap)}")
    w("")
    w("/* ESP32-S3 strapping pins (datasheet 2.4) crossed with board nets. */")
    w("typedef struct { int gpio; const char *net; const char *role; } strap_t;")
    w("static const strap_t STRAPS[] = {")
    for num, net, role in straps:
        w(f'    {{ {num}, "{net}", "{role}" }},')
    w("};")
    w(f"#define N_STRAPS {len(straps)}")
    w("")
    w("/* ── Diagnostic heartbeat LED ────────────────────────────────── */")
    w("/*")
    w(" * Blink-code contract, quoted from docs/diagnostic-leds-roadmap.md:")
    w(" *")
    w(' *   "Heartbeat firmware contract (bringup_test): 1 Hz steady = alive;')
    w(" *    N blinks + pause = subsystem N failed (2=SD, 3=display, 4=audio,")
    w(" *    5=buttons, 6=PSRAM) — telemetry stays the rich channel, the LED is")
    w(' *    the cable-free fallback."')
    w(" *")
    w(" * The table below is generated from BLINK_CODES in generate.py, which is")
    w(" * the single place the mapping is written down. A check joins a subsystem")
    w(" * by the prefix of its id, so a new sd.* or lcd.* check needs no edit here.")
    w(" */")
    if hb_gpio is None:
        w("/* board_config.h defines no " + HB_LED_NET + " net on this revision. */")
        w("#define HB_LED_PRESENT   0")
    else:
        w(f"#define HB_LED_PRESENT   1")
        w(f"#define HB_LED_PIN       {HB_LED_NET}")
        w(f"#define HB_LED_NUM       {hb_gpio}")
    w("")
    w("typedef struct {")
    w("    uint8_t     code;        /* number of short blinks              */")
    w("    const char *subsystem;   /* label, for the report line          */")
    w("    const char *id_prefix;   /* check ids this code speaks for      */")
    w("} hb_code_t;")
    w("")
    w("static const hb_code_t HB_CODES[] = {")
    for code, label, prefixes in BLINK_CODES:
        for prefix in prefixes:
            w(f'    {{ {code}, "{label}", "{prefix}" }},')
    w("};")
    w(f"#define N_HB_CODES  {sum(len(p) for _, _, p in BLINK_CODES)}")
    w(f"#define HB_CODE_MAX {max(code for code, _, _ in BLINK_CODES)}")
    w('#define HB_CODE_LEGEND "'
      + " ".join(f"{code}={label}" for code, label, _ in BLINK_CODES) + '"')
    w("")
    w(f'#define MODULE_VARIANT   "{variant}"')
    w(f"#define EXPECT_FLASH_MB  {flash_mb}")
    w(f"#define EXPECT_PSRAM_MB  {psram_mb}")
    w(f"#define EXPECT_USB_DP    {usb_dp}")
    w(f"#define EXPECT_USB_DN    {usb_dn}")
    w(f"#define I2S_DOUT_NUM     {i2s_dout}")
    w("")
    return "\n".join(L)


# ── the fixed part of the firmware ──────────────────────────────────────
...
static void chk_sd_cmd0(void)
{
    ...
    if (r1 == 0xFF) {
         ...
         * "Cd" on the TF-01A drawing) but this revision does not read it as
         * one — it lands on BTN_R because the track crosses the pad — so
         * there is no way to tell an empty socket from a broken MISO net,
         * and claiming either would be a guess. See CLAIM-006. */
...
def render(defines: dict[str, str], header_text: str) -> str:
    return (
        C_PREAMBLE
        + "\n"
        + c_tables(defines, header_text)
        + C_BODY
    )
...
def main() -> int:
    ...
    ap.add_argument("--check", action="store_true",
                    help="exit 1 if the generated file is stale instead of writing it")
    args = ap.parse_args()
    ...
    header_text = BOARD_CONFIG.read_text()
    defines = parse_defines(BOARD_CONFIG)

    try:
        content = render(defines, header_text)
    ...
    if args.check:
        if not OUTPUT.exists():
            print(f"STALE: {rel} does not exist", file=sys.stderr)
            return 1
        if OUTPUT.read_text() != content:
            print(f"STALE: {rel} does not match board_config.h — "
                  f"run python3 {Path(__file__).relative_to(PROJECT_ROOT)}",
                  file=sys.stderr)
            return 1
        print(f"OK: {rel} is in sync with board_config.h")
        return 0

    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(content)

    no_pu = nets_without_external_pullup(header_text)
    variant, flash_mb, psram_mb = parse_module_variant(SDKCONFIG_DEFAULTS.read_text())
    print(f"Wrote {rel} ({content.count(chr(10)) + 1} lines)")
    print(f"  module variant      : {variant} -> {flash_mb}MB flash, {psram_mb}MB PSRAM")
    print(f"  nets with no ext pull-up: {', '.join(sorted(no_pu)) or '(none found)'}")
    return 0
